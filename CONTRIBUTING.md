...
In an effort to not repeat bugs (and break less popular features), unit tests are run on Github Actions.  The goal of the tests are to be simple and to be placed in the `test/tests` directory where it will be automatically run.  Review existing tests for an example.
